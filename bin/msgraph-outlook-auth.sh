#!/bin/bash
# Microsoft Graph OAuth for personal Outlook.com accounts
# Uses the consumers tenant endpoint
# Usage: ./msgraph-outlook-auth.sh
...
echo "=============================================="
echo "Microsoft Graph OAuth - Outlook.com Account"
echo "=============================================="
echo ""
...
echo "$DEVICE_RESP" | python3 -c "import sys,json; d=json.load(sys.stdin); print('User Code:', d.get('user_code')); print('URL:', d.get('verification_url'))" 2>/dev/null
...
sed -i '' "s|MSGRAPH_TENANT_ID=.*|MSGRAPH_TENANT_ID=consumers|" .env
...
if echo "$TEST" | python3 -c "import sys,json; json.load(sys.stdin); print('valid')" 2>/dev/null; then
  TOTAL=$(echo "$TEST" | python3 -c "import sys,json; print(json.load(sys.stdin).get('totalItemCount', 'N/A'))")
  UNREAD=$(echo "$TEST" | python3 -c "import sys,json; print(json.load(sys.stdin).get('unreadItemCount', 'N/A'))")
  echo "Token works! Connected to Outlook.com"
